...
def runTAR3(I,V,VV,VVV,inode,smooth=False,vv=None):
    ...
    RUN_DIR=path.join(dir_path,'./tar3/sample/')
    lines=[]
    num_vars=len(V[0]) 
    if num_vars == 0:
        return None
    ...
    with open(path.join(dir_path,'./tar3/sample/data.data'),'w+') as handler:
        for line in lines:
            handler.write(','.join(line) + '\n')
        
    cp=subprocess.run([path.join(dir_path,"./tar3/source/tar3/tar3"), "data"],universal_newlines=True, stdout=subprocess.PIPE, cwd=RUN_DIR)
...
def generateTar3ConfingFile(arg_status,interest_indexes):
    '''
    this function build tar3 data.names file based on unit arg status and intersting index on char stars in unit
    '''
    file_path = path.abspath(__file__)
    dir_path = path.dirname(file_path)
    config_file=path.join(dir_path,'./tar3/sample/data.names')
