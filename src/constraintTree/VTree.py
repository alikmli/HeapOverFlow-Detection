...
from ..simprocedure import _strcpy_vul
from ..simprocedure import _strcat_vul
from ..simprocedure import _memcpy_vul
from ..simprocedure import _memmove_vul
from ..simprocedure import _memset_vul
from ..simprocedure import _sprintf_vul
from ..simprocedure import ExeFunc
from ..simprocedure import wcslen
import logging
from ..TypeUtils import *


class _VTree(angr.Analysis):
    def __init__(self,cfg_analyzer=None,solo=False):
        ...
        while len(self._simgr.active) > 0 :
            ...
            if len(act_state.globals['extra_const'])>0:
                ...
                if 'iswchar' in act_state.globals.keys() and act_state.globals['iswchar']==True:
                    iswchar=True
                vul_const=self.setUpVulConstraints(target_vul_state.vex,curr_wfCall,rhs,target_call_hist,iswchar)
    ...
    def setUpVulConstraints(self,curr_wfCall_vb,curr_wfCall,rhs,hist_call,iswchar=False):
        if curr_wfCall in self._func_wr_sites.keys():
            ...
            if props[0] == 'strcat':
                return self._getVulConstraintsForStrcat(curr_wfCall_vb,props,rhs,hist_call)
            if props[0] == 'memcpy' or props[0] == 'memmove':
                return self._getVulConstraintsForMemcpy(curr_wfCall_vb,props,rhs,hist_call,wchar=iswchar)
            if props[0] == 'memset':
                return self._getVulConstraintsForMemset(curr_wfCall_vb,props,rhs,hist_call,wchar=iswchar)
            if props[0] == 'sprintf':
                return self._getVulConstraintsForSprintf(curr_wfCall_vb,props,rhs,hist_call)
        
        
    def _getVulConstraintsForSprintf(self,vex,props,rhs,history_blocks):
        func_name,out_str=rhs
        if isinstance(out_str,str):
            dst_size=self._getSRCorDSTsize('sprintf',vex,props,'dst',history_blocks)
            if dst_size:
    ...
    def _getVulConstraintsForMemset(self,vex,props,rhs,history_blocks,wchar=False):
        func_name,num=rhs
        dst_size=self._getSRCorDSTsize('memset',vex,props,'dst',history_blocks,iswhar=wchar)
        if dst_size:
    ...
    def _getVulConstraintsForMemcpy(self,vex,props,rhs,history_blocks,wchar=False):       
        func_name,limit=rhs
        message='There is a Buffer Overflow in block {0} with target function {1}'.format(vex.addr,props[0])
        if isinstance(limit,int):
            dst_size=self._getSRCorDSTsize(func_name,vex,props,'dst',history_blocks,iswhar=wchar)
            if dst_size:
                if limit > dst_size:
                    self._is_static_vul=True
                    self._vulReports.append(message)   
            ...
            dst_size=self._getSRCorDSTsize(func_name,vex,props,'dst',history_blocks,iswhar=wchar)
            src_size=self._getSRCorDSTsize(func_name,vex,props,'src',history_blocks,iswhar=wchar)
            if dst_size and src_size:
                const=[limit <= src_size ,limit > dst_size]
                return (claripy.And(*const),message)
                
            
    def _getVulConstraintsForStrcpy(self,vex,props,rhs,history_blocks):
        message='There is a Buffer Overflow in block {} with target function strcpy'.format(vex.addr)
        if isinstance(rhs,str):
            dst_size=self._getSRCorDSTsize('strcpy',vex,props,'dst',history_blocks)
            if dst_size:
                len_rhs=len(rhs)
                if len_rhs >=  dst_size:
                    self._is_static_vul=True
            ...
            src_size=self._getSRCorDSTsize('strcpy',vex,props,'src',history_blocks)
            dst_size=self._getSRCorDSTsize('strcpy',vex,props,'dst',history_blocks)
            const=[]
            if src_size and dst_size:
                ...
                const.append(claripy.UGT(rhs ,dst_size *3 ))# len(src) > len(dst) * 3 
                #const.append(claripy.ULE(rhs,src_size))

                return (claripy.And(*const),message)
            elif dst_size:
                const.append(claripy.UGT(rhs ,dst_size *3 ))
                return (claripy.And(*const),message)
        

    def _getVulConstraintsForStrcat(self,vex,props,rhs,history_blocks):
        ...
        if isinstance(src_len,int):
            dst_size=self._getSRCorDSTsize('strcat',vex,props,'dst',history_blocks)
            if dst_size:
                if src_len > dst_size :
                    self._is_static_vul=True
                    self._vulReports.append(message)
            ...
            src_size=self._getSRCorDSTsize('strcat',vex,props,'src',history_blocks)
            dst_size=self._getSRCorDSTsize('strcat',vex,props,'dst',history_blocks)
            if src_size and dst_size:
    ...
    def _getSRCorDSTsize(self,func_name,vex,props,arg_type,history_blocks,iswhar=False):
        res_addr=None
        tmp_props_addr=[]
        is_inFunc=True
        ...
        if len(tmp_props_addr) == 1:
            ...
            if is_inFunc:
                ...
                malloc_size=self._malloc_boundry[tmp_props_addr.pop()]
                for addr in tmp_props_addr:
                    malloc_addr=self._malloc_boundry[addr]
                    if arg_type == 'src':
                        if malloc_addr >= malloc_size:
                            res_addr=addr
