...
from ..simprocedure import SimExtractParams
from .Tar3 import runTAR3,_correctInputs,_seperateValues,generateTar3ConfingFile
...
class Cover:
    ...
    def _extractUnitV(self,*args,args_indexes):
        ...
        if self._is_def:
            ...
            try:
                state.libc.buf_symbolic_bytes=BUF_SYMBOLIC_BYTES
                simgr=self.project.factory.simulation_manager(state)
                simgr.run()
            except ValueError:
                self._is_def=True
                state.libc.buf_symbolic_bytes=DEFAULT_BUF_SYMBOLIC_BYTES
                simgr=self.project.factory.simulation_manager(state)
                simgr.run()
